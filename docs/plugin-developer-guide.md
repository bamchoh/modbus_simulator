...
ホストアプリ (Wails)
  │
  ├─ PluginProcessManager  ← プラグイン起動・管理
  │    └─ plugin.json を走査して exe を起動
  │
  └─ gRPC クライアント
...
internal/infrastructure/myplugin/  # プロトコル本体（既存でも可）
    ├── factory.go
    ├── server.go
    └── datastore.go
...
import (
    ...
    "modbus_simulator/internal/infrastructure/myplugin"
)
...
`ProtocolType` はホストが各サーバーを識別するキーです。既存の `"modbus-tcp"`, `"modbus-rtu"`, `"modbus-ascii"`, `"opcua"` と衝突しない値を使ってください。
